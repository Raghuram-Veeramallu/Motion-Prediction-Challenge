...
from .modules import MCGBlock, HistoryEncoder, NormalMLP, Decoder, DecoderHandler, EM, MHA
...
class MultiPathPP(nn.Module):
    def __init__(self, config):
        ...
        encoder_layer_history = nn.TransformerEncoderLayer(d_model= 256, nhead=4)
        self._history_mcg_encoder_attn = nn.TransformerEncoder(encoder_layer_history, num_layers=4)

        encoder_layer_int = nn.TransformerEncoderLayer(d_model= 256, nhead=4)
        self._interaction_mcg_encoder_attn = nn.TransformerEncoder(encoder_layer_int, num_layers=4)

        encoder_layer_road = nn.TransformerEncoderLayer(d_model= 128, nhead=4)
        self._roadgraph_mcg_encoder_attn = nn.TransformerEncoder(encoder_layer_road, num_layers=4)
    ...
    def forward(self, data, num_steps):
        ...
        target_mcg_embedding_attn = self._history_mcg_encoder_attn(target_agents_embeddings)[-1]
        target_mcg_embedding_attn =  target_mcg_embedding_attn.unsqueeze(0)
        assert torch.isfinite(target_mcg_embedding_attn).all()
        ################################
        ...
        #target_agents_embeddings_repeated = target_agents_embeddings.repeat(36, 1, 1)
        inter_mcg_inputs = torch.cat([target_agents_embeddings, interaction_agents_embeddings], dim = 0)
        interaction_mcg_embedding_attn = self._interaction_mcg_encoder_attn(inter_mcg_inputs)[-1]
        interaction_mcg_embedding_attn = interaction_mcg_embedding_attn.unsqueeze(0)
        ...
        # print(f"target_mcg: {target_mcg_embedding_attn.shape}")
        # print(f"interaction_mcg: {interaction_mcg_embedding_attn.shape}")

        target_and_interaction_embedding = torch.cat(
            [target_mcg_embedding_attn, interaction_mcg_embedding_attn], axis=-1)
        ...
        road_graph_inputs = torch.cat([segment_embeddings, target_and_interaction_embedding_linear], dim = 0)
        roadgraph_mcg_embedding_attn = self._roadgraph_mcg_encoder_attn(road_graph_inputs)[-1]
        roadgraph_mcg_embedding_attn = roadgraph_mcg_embedding_attn.unsqueeze(0) 
        # assert torch.isfinite(roadgraph_mcg_embedding).all()
        assert torch.isfinite(roadgraph_mcg_embedding_attn).all()
        # print(f' roadgraph_mcg_embedding: {roadgraph_mcg_embedding.shape} vs {roadgraph_mcg_embedding_attn.shape}')
        #################

        final_embedding = torch.cat(
            [target_mcg_embedding_attn, interaction_mcg_embedding_attn, roadgraph_mcg_embedding_attn], dim=-1)
