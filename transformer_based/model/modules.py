...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(640, 8)
    ...
    def forward(self, target_scatter_numbers, target_scatter_idx, final_embedding, batch_size):
        ...
        #6 but final is 1... 6 + 1 = 7
        trans_inputs = torch.cat([self._learned_anchor_embeddings,final_embedding], dim=1)
        # print(f'trans_inputs shape: {trans_inputs.shape}')
        output = self.Encoder(trans_inputs)
        s_attn = output[:,:6,:]
        c_attn = output[:, 6, :]
        trajectories_embeddings_attn = s_attn * c_attn

        # assert torch.isfinite(trajectories_embeddings).all()
        assert torch.isfinite(trajectories_embeddings_attn).all()
        # print(f'trajectories_embedding: {trajectories_embeddings.shape}, vs {trajectories_embeddings_attn[0:6].shape}')
        ######################

        if self._return_embedding:
            return trajectories_embeddings_attn
        # 
        res = self._mlp_decoder(trajectories_embeddings_attn)
...
class HistoryEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(128, 8)
        self.Encoder = nn.TransformerEncoder(self.encoder_layer, 4)
    ...
    def forward(self, scatter_numbers, scatter_idx, lstm_data, lstm_data_diff, mcg_data):
        ...
        position_mcg_embedding_attn = self.Encoder(mcg_data)
        position_mcg_embedding_attn = torch.mean(position_mcg_embedding_attn, dim=1)
        position_mcg_embedding_attn = position_mcg_embedding_attn.unsqueeze(1)
        #print(f'position_mcg_embedding: {position_mcg_embedding.shape}, vs {position_mcg_embedding_attn.shape}')
        #################
        return torch.cat([
            position_lstm_embedding, position_diff_lstm_embedding, position_mcg_embedding_attn], axis=-1)
