...
from .utils import (
    ...
    get_normalize_data, get_filter_gt_addition_keys)
...
class MultiPathPPRenderer(Renderer):
    ...
    def _preprocess_data(self, data):
        ...
        data['modified/state/type'] = data['state/type']
        ...
        for key in get_filter_gt_addition_keys():
            data[f'modified/{key}'] = filter_valid(data[key], agents_with_any_validity_selector)
    ...
    def _prepare_agent_history(self, data):
        ...
        # print('XXXXXXXXXXXX')
        # print(data['state/future/velocity_x'].shape)
        # print(data['modified/state/future/velocity_x'].shape)
        # print('XXXXXXXXXXXX')

        future_states = np.stack([
            data['state/future/x'], data['state/future/y'],
            data['modified/state/future/length'], data['modified/state/future/width'],
            data['state/future/bbox_yaw'],
            data['modified/state/future/velocity_x'],
            data['modified/state/future/velocity_y']
        ], axis = -1)

        preprocessed_data["target/gt_future_states"] = future_states#.transpose(1, 2, 0)
        # print(preprocessed_data["target/gt_future_states"].shape)
    ...
    def render(self, data):
        ...
        # print('XXXXXXXXX')
        # print(data['modified/state/type'].shape)
        # print('XXXXXXXXX')
        ...
        for i in range(agent_history_info["history/xy"].shape[0]):
            ...
            scene_data = {
                ...
                "state/tracks_to_predict": data["state/tracks_to_predict"],
                "target/gt_future_states": agent_history_info["target/gt_future_states"][i][None,],
                "state/type": data["modified/state/type"],
