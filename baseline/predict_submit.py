...
def parse_args():
    ...
    parser.add_argument("--account-name", required=False, default="veera047@umn.edu")
    parser.add_argument("--authors", required=False, default="Hari Veeramallu,Mouhamad Ali Elamine,Ritik Parkar,Faizan Siddiqui")
    parser.add_argument("--method-name", required=False, default="mpp")
...
def main():
    ...
    with torch.no_grad():
        for data in tqdm(loader):
            ...
            for p, conf, aid, sid, c, y in zip(
                    ...
                    data["shift"], data["yaw"]):
                # if int(data["target/agent_type"][0]) != 3:
                #     continue
                if sid not in RES:
                    RES[sid] = list()
                ...
                if args.visualize:
                    ...
                    plt.savefig(os.path.join('./plots/new_plots', f'scid_{sid}__aid_{aid}__atype_{atype}.png'))
            num_steps += 1
    ...
    # Create a BytesIO object to hold the serialized submission
    submission_buffer = io.BytesIO(motion_challenge_submission.SerializeToString())

    # Create a TarFile object to write the compressed archive
    with tarfile.open("./results/submission.tar.gz", "w:gz") as tar:
        # Add the serialized submission to the archive
        tarinfo = tarfile.TarInfo(name="./results/baseline2.pb")
        tarinfo.size = len(motion_challenge_submission.SerializeToString())
        submission_buffer.seek(0)
        tar.addfile(tarinfo, submission_buffer)
