...
class MultiPathPPRenderer(Renderer):
    ...
    def _preprocess_data(self, data):
        ...
        for key in get_filter_gt_addition_keys():
            data[key] = filter_valid(data[key], agents_with_any_validity_selector)
        data["orig/state/tracks_to_predict"] = data["state/tracks_to_predict"].reshape(-1, 128)
        data["orig/state/tracks_to_predict"] = filter_valid(data["orig/state/tracks_to_predict"], agents_with_any_validity_selector)
    ...
    def _prepare_agent_history(self, data):
        ...
        future_states = np.stack([
            ...
            data['state/future/length'], data['state/future/width'],
            data['state/future/bbox_yaw'],
            data['state/future/velocity_x'],
            data['state/future/velocity_y']
        ], axis = -1)

        preprocessed_data["target/gt_future_states"] = future_states
    ...
    def render(self, data):
        ...
        for i in range(agent_history_info["history/xy"].shape[0]):
            ...
            scene_data = {
                ...
                "state/tracks_to_predict": data["orig/state/tracks_to_predict"],
                "target/gt_future_states": agent_history_info["target/gt_future_states"],
